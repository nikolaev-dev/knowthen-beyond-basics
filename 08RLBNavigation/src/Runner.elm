...
type Msg
    ...
    | BibInput String
    | Save
    | SaveResponce (Result Http.Error String)
...
save token model =
    let
        ...
        cmd =
            Http.send SaveResponce request
    in
        ( model, cmd )
...
validateName model =
    if String.isEmpty model.name then
        { model | nameError = Just "Name is Requiered" }
    else
...
validateLocation model =
    if String.isEmpty model.location then
        { model | locationError = Just "Location is Requiered" }
    else
